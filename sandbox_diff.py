...
class State:
    ...
    roi_expl:  chex.Array     # float [expl_grid_sz, expl_grid_sz]: ROI mask at exploration-map resolution
...
class SimpleSpreadMPE(MultiAgentEnv):
    ...
    def reset(self, key: chex.PRNGKey, tot_steps: int) -> Tuple[chex.Array, State]:
        ...
        roi_expl = self.get_roi_expl(hmap, es)
    ...
    def get_roi_expl(self, hmap: chex.Array, expl_side: int) -> chex.Array:
        """Return ROI mask aligned to the exploration-map grid."""
        roi_expl = jax.image.resize(hmap[1].astype(PRE), (expl_side, expl_side), "linear")
        return (roi_expl > PRE(0.5)).astype(PRE)
    ...
    def get_rewards(
        ...
        roi_mass = jnp.sum(state.roi_expl) + self.eps
        team_roi_coverage = jnp.clip(
            jnp.sum(state.expl_map * state.roi_expl[None, :, :], axis=(1, 2)) / roi_mass,
            PRE(0.0),
            PRE(1.0),
        )

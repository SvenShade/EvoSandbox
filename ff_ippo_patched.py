...
# Purpose: Extends InstaDeep's IPPO trainer to allow rendering.
...
def tree_all_finite(pytree: Any) -> chex.Array:
    """JAX-friendly finiteness check over a pytree of arrays."""
    leaves = [x for x in jax.tree_util.tree_leaves(pytree) if hasattr(x, "dtype")]
    if not leaves:
        return jnp.array(True)
    return jnp.all(jnp.stack([jnp.isfinite(x).all() for x in leaves]))
...
def final_episodes(config: DictConfig,
    ...
    env, _ = environments.make(config, add_global_state=False)
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                actor_grads_finite = tree_all_finite(actor_grads)
                critic_grads_finite = tree_all_finite(critic_grads)
                ...
                actor_params_finite = tree_all_finite(actor_new_params)
                critic_params_finite = tree_all_finite(critic_new_params)
                ...
                loss_info = {
                    ...
                    "entropy": entropy,
                    "actor_grads_finite": actor_grads_finite.astype(jnp.float32),
                    "critic_grads_finite": critic_grads_finite.astype(jnp.float32),
                    "actor_params_finite": actor_params_finite.astype(jnp.float32),
                    "critic_params_finite": critic_params_finite.astype(jnp.float32),
                }
...
def learner_setup(
    ...
    # Skip optimiser updates when gradients become non-finite.
    # This prevents a single bad minibatch from poisoning params permanently.
    actor_optim = optax.apply_if_finite(actor_optim, max_consecutive_errors=1)
    critic_optim = optax.apply_if_finite(critic_optim, max_consecutive_errors=1)
...
def run_experiment(_config: DictConfig) -> float:
    ...
    env, _      = environments.make(config)
    _, eval_env = environments.make(eval_config)

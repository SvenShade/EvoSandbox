...
class SimpleSpreadMPE(MultiAgentEnv):
    def __init__(
        ...
        **kwargs,
        ...
        self.scout_path_samples   = 5     # fixed line samples per step for path coverage
        self.scout_frontier_bonus = 0.15  # small bias toward coverage frontiers
        self.scout_overlap_penalty = 0.10 # discourage redundant overlapping scans
        self.scout_clearance_pref  = self.body_rad * 2.0  # preferred sensing clearance above terrain
        self.scout_clearance_sigma = self.body_rad * 1.0  # width of preferred sensing band
    ...
    def _render_debug_print(self, msg: str) -> None:
        if getattr(self, "render_debug", False):
            print(f"[render-debug] {msg}", flush=True)
    ...
    def path_splats(self, old_pos, new_pos, dones, hmap):
        """Bilinearly splat each alive agent's motion path onto the exploration grid.

        Splat mass is weighted by a preferred clearance band above local terrain,
        so scouting is most effective at a useful sensing altitude rather than
        monotonically favouring terrain-hugging flight.
        """
        s = int(self.env_size / self.expl_grid_sz)
        b = PRE(self.expl_grid_sz)
        eps = PRE(1e-4)
        ts = jnp.linspace(0.0, 1.0, self.scout_path_samples, dtype=PRE)
        pts = ((PRE(1.0) - ts)[None, :, None] * old_pos[:, None, :]
               + ts[None, :, None] * new_pos[:, None, :])
        gx = jnp.clip(pts[..., 0] / b, PRE(0.0), PRE(s - 1) - eps)
        gy = jnp.clip(pts[..., 1] / b, PRE(0.0), PRE(s - 1) - eps)

        flat_xy = pts[..., :2].reshape((-1, 2)) / PRE(self.env_size)
        terr_h = sample_hmap(hmap, flat_xy).reshape(pts.shape[:2]) * PRE(self.env_size)
        clearance = pts[..., 2] - terr_h
        pref = PRE(self.scout_clearance_pref)
        sigma = PRE(self.scout_clearance_sigma) + self.eps
        clearance_w = jnp.exp(-PRE(0.5) * jnp.square((clearance - pref) / sigma))

        def _splat_one(gx_i, gy_i, cw_i, alive):
            x0 = jnp.floor(gx_i).astype(jnp.int32)
            y0 = jnp.floor(gy_i).astype(jnp.int32)
            x1 = jnp.clip(x0 + 1, 0, s - 1)
            y1 = jnp.clip(y0 + 1, 0, s - 1)
            dx = gx_i - x0.astype(PRE)
            dy = gy_i - y0.astype(PRE)
            w00 = cw_i * (PRE(1.0) - dx) * (PRE(1.0) - dy)
            w10 = cw_i * dx * (PRE(1.0) - dy)
            w01 = cw_i * (PRE(1.0) - dx) * dy
            w11 = cw_i * dx * dy
            flat = jnp.zeros((s * s,), dtype=PRE)
            flat = flat.at[y0 * s + x0].add(w00)
            flat = flat.at[y0 * s + x1].add(w10)
            flat = flat.at[y1 * s + x0].add(w01)
            flat = flat.at[y1 * s + x1].add(w11)
            img = jnp.clip(flat.reshape((s, s)), min=PRE(0.0), max=PRE(1.0))
            return jnp.where(alive, img, PRE(0.0))

        return jax.vmap(_splat_one)(gx, gy, clearance_w, ~dones)

    def scouting_update(self, old_pos, new_pos, dones, old_expl_map, hmap):
        """Update team exploration maps and assign per-agent scouting credit.

        Credit is assigned per cell in proportion to each agent's splat mass on the
        newly covered area, so overlapping scouts split reward rather than double count it.
        """
        decayed_expl = old_expl_map * PRE(self.expl_decay)
        agent_splats = self.path_splats(old_pos, new_pos, dones, hmap)         # [N, H, W]
        team_splats = jnp.zeros_like(decayed_expl).at[self.team_ids].add(agent_splats)
        expl_map = jnp.clip(decayed_expl + team_splats, min=PRE(0.0), max=PRE(1.0))
        delta = jnp.maximum(expl_map - decayed_expl, PRE(0.0))

        team_splats_by_agent = team_splats[self.team_ids]
        delta_by_agent = delta[self.team_ids]
        cover_credit = jnp.sum(
            delta_by_agent * agent_splats / (team_splats_by_agent + self.eps),
            axis=(1, 2),
        )

        unexplored = PRE(1.0) - decayed_expl
        vdiff = jnp.abs(unexplored[:, 1:, :] - unexplored[:, :-1, :])
        hdiff = jnp.abs(unexplored[:, :, 1:] - unexplored[:, :, :-1])
        frontier = (
            jnp.pad(vdiff, ((0, 0), (1, 0), (0, 0)))
            + jnp.pad(vdiff, ((0, 0), (0, 1), (0, 0)))
            + jnp.pad(hdiff, ((0, 0), (0, 0), (1, 0)))
            + jnp.pad(hdiff, ((0, 0), (0, 0), (0, 1)))
        ) / PRE(4.0)
        frontier = jnp.clip(frontier, min=PRE(0.0), max=PRE(1.0))

        agent_mass = jnp.sum(agent_splats, axis=(1, 2))
        frontier_credit = jnp.sum(
            agent_splats * frontier[self.team_ids], axis=(1, 2)
        ) / (agent_mass + self.eps)

        overlap_with_others = jnp.clip(team_splats_by_agent - agent_splats, min=PRE(0.0), max=PRE(1.0))
        overlap_penalty = jnp.sum(
            agent_splats * overlap_with_others, axis=(1, 2)
        ) / (agent_mass + self.eps)

        scouting_term = (
            cover_credit
            + PRE(self.scout_frontier_bonus) * frontier_credit
            - PRE(self.scout_overlap_penalty) * overlap_penalty
        )
        scouting_term = jnp.where(~dones, scouting_term, PRE(0.0))
        return expl_map.astype(PRE), scouting_term.astype(PRE)
    ...
    def step_env(self, key: chex.PRNGKey, state: State, actions: dict):
        ...
        # Update exploration map and per-agent scouting credit from post-move
        # motion paths. This rewards the team for expanding covered territory,
        # splits overlapping credit fairly, and mildly favours frontier expansion.
        expl_map, scouting_terms = self.scouting_update(
            state.p_pos, p_pos, state.done, state.expl_map, state.hmap
        )
        ...
        rewards = self.get_rewards(
            sqr_dist, aerials, net_hits, g_kills, agnt_done, done_ago, state, scouting_terms,
            train=not self.eval_mode)
    ...
    def get_actor_critic_obs(
        ...
        self_tem = self.team_ids[:, None].astype(PRE) * PRE(2.0) - PRE(1.0)
    ...
    def get_rewards(
            self, sqr_dist, aerials, net_hits, kills, agnt_done, done_ago, state, scouting_terms, train=True,
            ) -> Dict[str, chex.Array]:
        # Scaling preparation.
        ...
        def _rew(i: int):
            ...
            # SCOUTING ----------------------------------------------------- #
            # Reward agents for their fractional contribution to newly covered
            # territory, with a small frontier bonus and overlap penalty.
    ...
    def _render_view(
        ...
        dbg = lambda m: self._render_debug_print(f"{debug_label or view}: {m}")
        dbg(f"start window_size={window_size} transition={transition}")
        # Initialise plotter.
        dbg("creating plotter")
        ...
        dbg("lights/trails/labels added")

        # Render and return.
        dbg("calling screenshot")
        img = pl.screenshot(transparent_background=not background, return_img=True)
        dbg("screenshot returned")
        pl.close()
        dbg(f"closed plotter img_shape={img[..., :3].shape}")
    ...
    def render_vtk(
        ...
        debug_prev = getattr(self, "render_debug", False)
        self.render_debug = debug_prev or verbose
        self._render_debug_print(f"render_vtk start step={step_str} azim={azim:.2f} h={h}")
        ...
        self._render_debug_print(f"terrain grid built hmap_shape={np.asarray(state.hmap).shape}")
        ...
        self._render_debug_print("agent colors computed")
        ...
        self._render_debug_print(f"camera setup pos_shape={pos.shape} hist_shape={hist.shape}")
        ...
        self._render_debug_print("glyphs built")
        ...
        self._render_debug_print("all PyVista views rendered")
        ...
        self._render_debug_print("info and exploration panels rendered")
        ...
        self._render_debug_print("titles and borders drawn")
        ...
        out = np.concatenate([per_img, ort_img, inf_img], axis=1).astype(np.uint8)
        self._render_debug_print(f"render_vtk done out_shape={out.shape}")
        self.render_debug = debug_prev
        return out

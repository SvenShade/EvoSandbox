...
class SwarmVSwarmMPE(SimpleMPE):
    ...
    def reset(self, key: chex.PRNGKey) -> State:
        ...
        return State(**state.__dict__, active=active, done=done, battery=battery)
    ...
    def step_env(self, key: chex.PRNGKey, state: State, actions: Dict[str, chex.Array]):
        ...
        move_cost = self.move_cost_coef * jnp.sum(jnp.abs(ax_ay), axis=1)
        ...
        # Parent physics
        next_state, _, info, metrics = super().step_env(key, state, move_actions)
        ...
        # Collisions (tag)
        p_pos_agents = next_state.p_pos[: self.N]
        ...
        done_all = (team_a_dead | team_b_dead) | (next_state.step >= self.max_steps)

        per_agent_done = ~new_active
        done_vec = jnp.where(per_agent_done, True, done_all)

        next_state = State(**next_state.__dict__, active=new_active, done=done_vec, battery=new_battery)
        ...
        return obs, next_state, rewards_dict, dones, info
    ...
    def get_obs_tensor(self, state: State) -> chex.Array:
        ...
        rel_vel = jnp.broadcast_to(p_vel_agents[:, None, :, :], rel_pos.shape)
    ...
    def _rewards_vec(
        ...
        spread_collision_pen = self.local_collision_penalty * jnp.sum(coll_mat & act_pair, axis=1).astype(jnp.float32)
        ...
        fire_term = (self.fire_enemy_reward * fire_enemy_counts +
                     self.fire_friend_penalty * fire_friend_counts) * active.astype(jnp.float32)
    ...
    # Dict wrapper (recompute if mats missing)
    def rewards(self, state: State, coll_mat=None, tag_hits_enemy=None, aoe_hits=None) -> Dict[str, chex.Array]:
        if coll_mat is None:
            p_pos_agents = state.p_pos[: self.N]
            diff = p_pos_agents[:, None, :] - p_pos_agents[None, :, :]
            dist_sq = jnp.sum(diff * diff, axis=-1)

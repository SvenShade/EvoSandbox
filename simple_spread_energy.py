...
# ENVIRONMENT
ONE_SWARM = True  # Whether agents form one or two teams
KILL_BOX  = False # Whether to remove terrain/obstacles and keep agents only
ATTITUDES = True  # Whether to use attitudes
ATT_OVERR = None  # jnp.array([[0.0, 0.0, 1.0]]) # Override operator attitudes if not None.
...
SWARM_BASELINE = False # If True, agent motion is controlled by the local swarming baseline.
SWARM_BASELINE_TEAM = "all" # Options: all, team_0, team_1.
SWARM_BASELINE_BLEND = 1.0 # 1.0 fully replaces learned spatial/attitude actions.
...
RNDR_LITE = True  # Whether to use a lightweight renderer
...
class State:
    ...
    critic_obs: chex.Array # float [N, obs_dim]
    step:      int
    tot_steps: int
    warmup:    float
    warmup_multi: float
...
class SimpleSpreadMPE(MultiAgentEnv):
    def __init__(
        ...
        **kwargs,
        ...
        self.dirich_alpha = dirich_alpha # Alpha for Dirichlet (used in sampling operator preferences)
        ...
        self.pgon_accel     = self.accel
        self.pgon_max_speed = self.max_speed
        ...
        # or clay-pigeon entities in small and large scenarios.
        self.use_swarm_baseline = bool(kwargs.get("use_swarm_baseline", SWARM_BASELINE))
        self.swarm_baseline_team = str(kwargs.get("swarm_baseline_team", SWARM_BASELINE_TEAM))
        self.swarm_baseline_blend = float(kwargs.get("swarm_baseline_blend", SWARM_BASELINE_BLEND))
        valid_baseline_teams = ("all", "team_0", "team_1")
        if self.swarm_baseline_team not in valid_baseline_teams:
            raise ValueError(
                f"Unknown swarm_baseline_team={self.swarm_baseline_team}. "
                f"Expected one of {valid_baseline_teams}."
            )
        self.ctrl_alpha      = float(kwargs.get("swarm_alpha", 3.0))
        self.ctrl_avoid_rad  = float(kwargs.get(
            "swarm_avoid_rad", max(self.body_rad * 6.0, self.view_rad * 0.18)))
        self.ctrl_align_rad  = float(kwargs.get("swarm_align_rad", self.view_rad))
        self.ctrl_cohes_rad  = float(kwargs.get("swarm_cohesion_rad", self.view_rad))
        self.ctrl_fov_cos    = jnp.cos(float(kwargs.get("swarm_fov", 2.0 * math.pi / 3.0)))
        self.ctrl_cruise_spd = float(kwargs.get("swarm_cruise_speed", self.max_speed * 0.45))
        self.ctrl_avoid_gain = float(kwargs.get("swarm_avoid_gain", self.accel * 0.95))
        self.ctrl_align_gain = float(kwargs.get("swarm_align_gain", self.accel * 0.35))
        self.ctrl_cohes_gain = float(kwargs.get("swarm_cohesion_gain", self.accel * 0.25))
        self.ctrl_speed_gain = float(kwargs.get("swarm_speed_gain", 0.45))
        self.ctrl_target_gain = float(kwargs.get("swarm_target_gain", self.accel * 0.80))
        self.ctrl_threat_gain = float(kwargs.get("swarm_threat_gain", self.pgon_accel * 1.25))
        self.ctrl_terr_gain  = float(kwargs.get("swarm_terrain_gain", self.accel * 1.25))
        self.ctrl_edge_gain  = float(kwargs.get("swarm_edge_gain", self.accel * 0.50))
        self.ctrl_safe_alt   = float(kwargs.get("swarm_safe_alt", self.body_rad * 2.0))
        self.ctrl_edge_margin = float(kwargs.get("swarm_edge_margin", self.view_rad))
        ...
        self.obs_dim = int(ego_dim + nbor_dim + pgon_dim + pair_dim + terrain_dim)
    ...
    def interactions(self, state: State):
        ...
        raw_diff_pos = state.p_pos[None, :] - state.p_pos[:, None]  # [N,N,dim_p]
        raw_sqr_dist = jnp.sum(raw_diff_pos ** 2, axis=-1)

        # Valid interactions are between distinct, active agents.
        valid_mask = ~((state.done[None, :] | state.done[:, None])
                       | jnp.eye(self.num_agnt, dtype=jnp.bool_))
        visib_mask = raw_sqr_dist < self.view_rad_sqr
        actor_mask = visib_mask & valid_mask

        diff_pos = jnp.where(actor_mask[..., None], raw_diff_pos, 0.0).astype(PRE)
        sqr_dist = jnp.where(actor_mask, raw_sqr_dist, 0.0).astype(PRE)
        critic_diff_pos = jnp.where(valid_mask[..., None], raw_diff_pos, 0.0).astype(PRE)
        critic_sqr_dist = jnp.where(valid_mask, raw_sqr_dist, 0.0).astype(PRE)

        # Calculate relative positions and squared dists from agents to clay pigeons.
        raw_pg_diff_pos = state.pg_pos[None, :, :] - state.p_pos[:, None, :]
        raw_pg_sqr_dist = jnp.sum(raw_pg_diff_pos ** 2, axis=-1)
        pg_valid_mask = ((~state.done)[:, None] & (~state.pg_done)[None, :])
        pg_visib_mask = raw_pg_sqr_dist < self.view_rad_sqr
        pg_actor_mask = pg_valid_mask & pg_visib_mask

        pg_diff_pos = jnp.where(pg_actor_mask[..., None], raw_pg_diff_pos, 0.0).astype(PRE)
        pg_sqr_dist = jnp.where(pg_actor_mask, raw_pg_sqr_dist, 0.0).astype(PRE)
        critic_pg_diff_pos = jnp.where(pg_valid_mask[..., None], raw_pg_diff_pos, 0.0).astype(PRE)
        critic_pg_sqr_dist = jnp.where(pg_valid_mask, raw_pg_sqr_dist, 0.0).astype(PRE)
    ...
    def _clip_norm(self, x, max_norm):
        norm = jnp.linalg.norm(x, axis=-1, keepdims=True)
        scale = jnp.minimum(PRE(1.0), max_norm / (norm + self.eps))
        return x * scale

    def _motion_heading(self, vel, ori):
        fallback = jnp.broadcast_to(
            jnp.array([1.0, 0.0, 0.0], dtype=PRE),
            vel.shape,
        )
        vel_active = jnp.linalg.norm(vel, axis=-1, keepdims=True) > self.eps
        ori_active = jnp.linalg.norm(ori, axis=-1, keepdims=True) > self.eps
        heading_src = jnp.where(ori_active, ori, fallback)
        heading_src = jnp.where(vel_active, vel, heading_src)
        return self._unit(heading_src)

    def _external_energy_force(
            self,
            pos,
            done,
            other_pos,
        ...
        r_hat = diff / dist[..., None]
        weights = jnp.where(valid, (1.0 - dist / radius) ** self.ctrl_alpha, 0.0)
        force = sign * jnp.sum(r_hat * weights[..., None], axis=1) * gain
        return self._clip_norm(force, gain)

    def _terrain_edge_force(self, pos, land_heights, done, max_force):
        # Push up near terrain while adding enough lift to counter gravity.
        alt = pos[:, -1] / self.env_size - land_heights
        terr_push = jnp.clip((self.ctrl_safe_alt / self.env_size - alt)
                             / (self.ctrl_safe_alt / self.env_size + self.eps),
                             min=0.0, max=1.0)

        # Push inward near the world boundary.
        edge_margin = self.ctrl_edge_margin
        ceiling = self.env_size * self.height_lim
        low_edge = jnp.clip((edge_margin - pos) / (edge_margin + self.eps), 0.0, 1.0)
        high_xy = jnp.clip((pos[:, :2] - (self.env_size - edge_margin))
                           / (edge_margin + self.eps), 0.0, 1.0)
        high_z = jnp.clip((pos[:, 2] - (ceiling - edge_margin))
                          / (edge_margin + self.eps), 0.0, 1.0)
        edge_push = jnp.stack([
        ...
        force = edge_push * self.ctrl_edge_gain
        force = force.at[:, 2].add(self.gravity + terr_push * self.ctrl_terr_gain)
        force = jnp.where(done[:, None], 0.0, force)
        return self._clip_norm(force, max_force)

    def _swarming_energy_force(
            self,
            pos,
            vel,
            ori,
        ...
        # Reynolds-style local rules. Avoidance is the negative gradient of a
        # compact-support distance energy; alignment and cohesion steer toward
        # local low-energy headings without requiring centralised state at
        # execution time.
        ...
        valid = ((~done)[:, None]
                 & (~done)[None, :]
                 & (~jnp.eye(pos.shape[0], dtype=jnp.bool_))
                 & (sqr_dist > 0.0))
        heading = self._motion_heading(vel, ori)
        in_view = jnp.sum(r_hat * heading[:, None, :], axis=-1) >= self.ctrl_fov_cos

        avoid_mask = valid & (dist < self.ctrl_avoid_rad)
        avoid_w = jnp.where(
            avoid_mask,
            (1.0 - dist / self.ctrl_avoid_rad) ** (self.ctrl_alpha - 1.0),
            0.0,
        )
        avoid = -jnp.sum(r_hat * avoid_w[..., None], axis=1) * self.ctrl_avoid_gain

        nbor_mask = valid & in_view & (dist < self.ctrl_align_rad)
        nbor_w = jnp.where(
            nbor_mask,
            (1.0 - dist / self.ctrl_align_rad) ** self.ctrl_alpha,
            0.0,
        )
        nbor_count = jnp.sum(nbor_w, axis=1, keepdims=True)
        avg_heading = jnp.sum(heading[None, :, :] * nbor_w[..., None], axis=1)
        avg_heading = avg_heading / (nbor_count + self.eps)
        align = (self._unit(avg_heading) - heading) * self.ctrl_align_gain
        align = jnp.where(nbor_count > 0.0, align, 0.0)

        cohes_mask = valid & in_view & (dist < self.ctrl_cohes_rad)
        cohes_w = jnp.where(
            cohes_mask,
            (1.0 - dist / self.ctrl_cohes_rad) ** self.ctrl_alpha,
            0.0,
        )
        cohes_count = jnp.sum(cohes_w, axis=1, keepdims=True)
        center = jnp.sum(diff * cohes_w[..., None], axis=1) / (cohes_count + self.eps)
        center_dist = jnp.linalg.norm(center, axis=-1, keepdims=True)
        cohes = self._unit(center) * jnp.clip(
            center_dist / (self.ctrl_cohes_rad + self.eps),
            0.0,
            1.0,
        ) * self.ctrl_cohes_gain
        cohes = jnp.where(cohes_count > 0.0, cohes, 0.0)

        target = self._external_energy_force(
            pos, done, target_pos, target_done, self.view_rad, target_gain, 1.0)
        threat = self._external_energy_force(
            pos, done, threat_pos, threat_done, self.view_rad, threat_gain, -1.0)
        terrain_edge = self._terrain_edge_force(pos, land_heights, done, max_force)

        force = avoid + align + cohes + target + threat + terrain_edge
        desired_heading = jnp.where(
            jnp.linalg.norm(force, axis=-1, keepdims=True) > self.eps,
            self._unit(force),
            heading,
        )
        cruise = (desired_heading * jnp.minimum(self.ctrl_cruise_spd, max_speed) - vel)
        force = force + cruise * self.ctrl_speed_gain
        force = jnp.where(done[:, None], 0.0, force)
        return self._clip_norm(force, max_force).astype(PRE)

    def _baseline_agent_mask(self):
        if self.swarm_baseline_team == "team_0":
            return self.team_ids == 0
        if self.swarm_baseline_team == "team_1":
            return self.team_ids == 1
        return jnp.full((self.num_agnt,), True)

    def swarming_baseline_actions(self, state: State, land_heights):
        force = self._swarming_energy_force(
            state.p_pos,
            state.p_vel,
            state.p_ori,
            state.done,
            ...
            state.pg_pos,
            state.pg_done,
            self.ctrl_target_gain,
            jnp.zeros((0, self.dim_p), dtype=PRE),
            jnp.zeros((0,), dtype=jnp.bool_),
            0.0,
        )
        xyz = jnp.clip(force / self.accel, -1.0, 1.0)
        target_att = state.op_att[self.team_ids.astype(jnp.int32)]
    ...
    def step_env(self, key: chex.PRNGKey, state: State, actions: dict):
        ...
        # Move agents given actions. The optional swarming baseline can replace
        # learned actions for all agents or a single team, enabling direct
        # learned-policy vs. decentralised-controller comparisons.
        action_arr = self.actions_to_array(actions)
        if self.use_swarm_baseline:
            baseline_actions = self.swarming_baseline_actions(state, land_heights)
            baseline_mask = self._baseline_agent_mask()[:, None]
            blend = jnp.asarray(self.swarm_baseline_blend, dtype=PRE)
            action_arr = jnp.where(
                baseline_mask,
                action_arr * (PRE(1.0) - blend) + baseline_actions * blend,
                action_arr,
            )
        xyz, att, raw_actions = self.set_actions(action_arr, state.done)
        key, key_w = jax.random.split(key)
        p_pos, p_vel = self._world_step(key_w, state, xyz)

        # Move clay pigeons with the same local swarming controller.
        ...
        state = state.replace(
            ...
            critic_obs=state.critic_obs.astype(PRE),
        ...
        obs, critic_obs = self.get_actor_critic_obs(
            diff_pos, sqr_dist, critic_diff_pos, critic_sqr_dist,
            pg_diff_pos, pg_sqr_dist, critic_pg_diff_pos, critic_pg_sqr_dist,
            aerials, state,
        )
        rewards = self.get_rewards(sqr_dist, aerials, net_hits, g_kills, agnt_done, done_ago, state, train=not self.eval_mode)
        state   = state.replace(g_kills=g_kills.astype(int), critic_obs=critic_obs.astype(PRE))
    ...
    def reset(self, key: chex.PRNGKey, tot_steps: int) -> Tuple[chex.Array, State]:
        ...
        # Randomise operator intent/s. Sample from Dirichlet, unless ATT_OVERR is specified.
        shape  = (1,) if ONE_SWARM else (2,)
        alpha  = jnp.full(self.dim_a, self.dirich_alpha)
        op_att = ATT_OVERR if ATT_OVERR is not None else jax.random.dirichlet(key_att, alpha, shape)
        ...
        state = State(
            ...
            critic_obs=jnp.zeros((self.num_agnt, self.obs_dim), dtype=PRE),
            step=0,
        ...
        state = state.replace(critic_obs=critic_obs.astype(PRE))
        return obs, state

    # OBSERVATIONS AND REWARDS --------------------------------------------- #

    def get_actor_critic_obs(
            self,
            diff_pos,
            sqr_dist,
            critic_diff_pos,
        ...
        # Critic observations share the same schema, but are built from a shared
        # globally-nearest entity basis. The actor view is obtained by masking that
        # basis down to entities within self.view_rad, avoiding a second top-k pass.

        # Calculate ray angles, then rotate by agent yaw.
        W  = self.view_width
        cx = (PRE(W) - PRE(1.0)) * PRE(0.5)
        base_theta = PRE(2.0) * jnp.pi * (jnp.arange(W, dtype=PRE) / PRE(W))
        yaw = jnp.arctan2(state.p_ori[:, 1], state.p_ori[:, 0]).astype(PRE)
        theta = (yaw[:, None] + base_theta[None, :]).astype(PRE)[:, :, None]

        # Samples along each ray from edge->center->edge (t in [-1, 1]).
        t = jnp.linspace(PRE(-1.0), PRE(1.0), W, dtype=PRE)[None, None, :]

        # Ray coordinates in patch pixel space: (x,y) in [0, W-1]
        x = cx + (t * jnp.cos(theta) * cx)
        y = cx + (t * jnp.sin(theta) * cx)
        ...
        star_flat = star.reshape((self.num_agnt, W * W))

        # Shared nearest-neighbour basis over all valid entities.
        pad_nbor = max(0, self.obs_slots - self.num_agnt)
        valid_nbor_dists = jnp.where(critic_sqr_dist > 0.0, critic_sqr_dist, jnp.inf)
        valid_nbor_dists = jnp.pad(valid_nbor_dists, ((0, 0), (0, pad_nbor)), constant_values=jnp.inf)
        valid_nbor_diff = jnp.pad(critic_diff_pos, ((0, 0), (0, pad_nbor), (0, 0)))
        ...
        _, nbor_idxs = jax.lax.top_k(-valid_nbor_dists, self.obs_slots)
        nbor_dists = jnp.take_along_axis(valid_nbor_dists, nbor_idxs, axis=-1)
        nbor_present = jnp.isfinite(nbor_dists)
        nbor_visible = nbor_present & (nbor_dists < self.view_rad_sqr)
        nbor_pos_raw = jnp.take_along_axis(valid_nbor_diff, nbor_idxs[..., None], axis=1)
        nbor_vel_raw = p_vel_pad[nbor_idxs] - state.p_vel[:, None, :]
        nbor_team_delta = jnp.abs(team_ids_pad[nbor_idxs] - self.team_ids[:, None]).astype(PRE)
        nbor_att_raw = (attitudes_pad[nbor_idxs] - state.attitudes[:, None, :]) * (PRE(1.0) - nbor_team_delta)[..., None]
        nbor_fire_raw = fire_pad[nbor_idxs] * PRE(2.0) - PRE(1.0)
        nbor_team_raw = nbor_team_delta

        pad_pgon = max(0, self.obs_slots - self.num_pgon)
        valid_pgon_dists = jnp.where(critic_pg_sqr_dist > 0.0, critic_pg_sqr_dist, jnp.inf)
        valid_pgon_dists = jnp.pad(valid_pgon_dists, ((0, 0), (0, pad_pgon)), constant_values=jnp.inf)
        valid_pgon_diff = jnp.pad(critic_pg_diff_pos, ((0, 0), (0, pad_pgon), (0, 0)))
        pg_vel_pad = jnp.pad(state.pg_vel, ((0, pad_pgon), (0, 0)))
        pg_batt_pad = jnp.pad(state.pg_batt, (0, pad_pgon))

        _, pgon_idxs = jax.lax.top_k(-valid_pgon_dists, self.obs_slots)
        pgon_dists = jnp.take_along_axis(valid_pgon_dists, pgon_idxs, axis=-1)
        pgon_present = jnp.isfinite(pgon_dists)
        pgon_visible = pgon_present & (pgon_dists < self.view_rad_sqr)
        pgon_pos_raw = jnp.take_along_axis(valid_pgon_diff, pgon_idxs[..., None], axis=1)
        pgon_vel_raw = pg_vel_pad[pgon_idxs] - state.p_vel[:, None, :]
        pgon_batt_raw = (pg_batt_pad[pgon_idxs] / self.pgon_init_batt) * PRE(2.0) - PRE(1.0)

        sqrt3 = jnp.sqrt(PRE(3.0))

        def _format_nbors(pos_raw, vel_raw, att_raw, fre_raw, tem_raw, slot_mask, pos_scale):
            mask3 = slot_mask[..., None]
            pos = jnp.where(mask3, pos_raw / pos_scale, PRE(0.0))
            vel = jnp.where(mask3, vel_raw / self.max_speed, PRE(0.0))
            att = jnp.where(mask3, att_raw, PRE(0.0))
            fre = jnp.where(slot_mask, fre_raw, PRE(0.0))
            tem = jnp.where(slot_mask, tem_raw, PRE(0.0))

            pos_norm = jnp.linalg.norm(pos, axis=-1)
            vel_norm = jnp.linalg.norm(vel, axis=-1)
            ...
            r_hat = pos / (pos_norm[..., None] + self.eps)
            radial = jnp.sum(r_hat * vel, axis=-1)
            nbor_rdot = jnp.clip(radial / (sqrt3 + self.eps), -1.0, 1.0)
            u_perp = vel - radial[..., None] * r_hat
            tang = jnp.linalg.norm(u_perp, axis=-1)
            nbor_tan = (jnp.clip(tang / (sqrt3 + self.eps), 0.0, 1.0) * PRE(2.0) - PRE(1.0))

            nbor_inv = jnp.stack([
                jnp.where(slot_mask, nbor_rng, PRE(0.0)),
                jnp.where(slot_mask, nbor_spd, PRE(0.0)),
                jnp.where(slot_mask, nbor_rdot, PRE(0.0)),
                jnp.where(slot_mask, nbor_tan, PRE(0.0)),
            ], axis=-1)

            dmat = jnp.linalg.norm(pos[:, :, None, :] - pos[:, None, :, :], axis=-1)
            pair_d = dmat[:, self.pair_i, self.pair_j]
            pair_m = slot_mask[:, self.pair_i] & slot_mask[:, self.pair_j]
            pair_feat = (jnp.clip(pair_d / (PRE(2.0) * sqrt3 + self.eps), 0.0, 1.0) * PRE(2.0) - PRE(1.0))
            pair_feat = jnp.where(pair_m, pair_feat, PRE(0.0))

            nbor = jnp.concatenate([
                pos,
                vel,
                att,
                fre[..., None],
                tem[..., None],
                nbor_inv,
            ], axis=-1).reshape((self.num_agnt, -1))
            return jnp.concatenate([nbor, pair_feat], axis=-1)

        def _format_pgons(pos_raw, vel_raw, batt_raw, slot_mask, pos_scale):
            mask3 = slot_mask[..., None]
            pos = jnp.where(mask3, pos_raw / pos_scale, PRE(0.0))
            vel = jnp.where(mask3, vel_raw / self.pgon_max_speed, PRE(0.0))
            batt = jnp.where(slot_mask, batt_raw, PRE(0.0))
            ...
            pg_inv = jnp.stack([
                jnp.where(slot_mask, (jnp.clip(pos_norm / (sqrt3 + self.eps), 0.0, 1.0) * PRE(2.0) - PRE(1.0)), PRE(0.0)),
                jnp.where(slot_mask, (jnp.clip(vel_norm / (sqrt3 + self.eps), 0.0, 1.0) * PRE(2.0) - PRE(1.0)), PRE(0.0)),
                jnp.where(slot_mask, jnp.clip(radial / (sqrt3 + self.eps), -1.0, 1.0), PRE(0.0)),
                jnp.where(slot_mask, (jnp.clip(tang / (sqrt3 + self.eps), 0.0, 1.0) * PRE(2.0) - PRE(1.0)), PRE(0.0)),
            ], axis=-1)
        ...
        self_pos = (state.p_pos / self.env_size) * PRE(2.0) - PRE(1.0)
        self_vel = state.p_vel / self.max_speed
        self_att = state.attitudes * PRE(2.0) - PRE(1.0)
        self_fre = state.fire[:, None].astype(PRE) * PRE(2.0) - PRE(1.0)
        self_frd = (state.fire_ago[:, None] / self.fire_refr).astype(PRE) * PRE(2.0) - PRE(1.0)
        self_tem = (self.team_ids[:, None].astype(PRE) * PRE(2.0) - PRE(1.0)) * HIDE_SEEK
        self_bat = (state.batt[:, None] / self.init_battery) * PRE(2.0) - PRE(1.0)
        ...
        actor_nbor = _format_nbors(
            nbor_pos_raw, nbor_vel_raw, nbor_att_raw, nbor_fire_raw, nbor_team_raw,
            nbor_visible, PRE(self.view_rad)
        )
        critic_nbor = _format_nbors(
            nbor_pos_raw, nbor_vel_raw, nbor_att_raw, nbor_fire_raw, nbor_team_raw,
            nbor_present, PRE(self.env_size)
        )
        actor_pgon = _format_pgons(
            pgon_pos_raw, pgon_vel_raw, pgon_batt_raw, pgon_visible, PRE(self.view_rad)
        )
        critic_pgon = _format_pgons(
            pgon_pos_raw, pgon_vel_raw, pgon_batt_raw, pgon_present, PRE(self.env_size)
        )
        aeri_ = star_flat * PRE(2.0) - PRE(1.0)

        actor_obs_arr = jnp.concatenate([self_, actor_nbor, actor_pgon, aeri_], axis=-1).astype(PRE)
        critic_obs_arr = jnp.concatenate([self_, critic_nbor, critic_pgon, aeri_], axis=-1).astype(PRE)
        actor_obs = {a: actor_obs_arr[i] for i, a in enumerate(self.agents)}
        return actor_obs, critic_obs_arr

    def get_obs(self, diff_pos, sqr_dist, pg_diff_pos, pg_sqr_dist, aerials, state: State) -> Dict[str, chex.Array]:
        actor_obs, _ = self.get_actor_critic_obs(
            diff_pos,
            sqr_dist,
            diff_pos,
            sqr_dist,
    ...
    def actions_to_array(self, actions: Dict):
        if isinstance(actions, dict):
            return jnp.array([actions[i] for i in self.agents]).reshape((self.num_agnt, -1))
        return jnp.asarray(actions).reshape((self.num_agnt, -1))

    # Process actions, zeroing where agents are done.
    def set_actions(self, actions: Dict, done: chex.Array):
        actions = self.actions_to_array(actions) * ~done[:, None] * ~self.frozen[:, None]
    ...
    def _render_view(
        ...
        if view == 'top':
            ...
            if toggle_bounds:
                pl.show_bounds(
                    ...
                    ztitle='z', 
                    color='white')
        elif view == 'perspective' and not RNDR_LITE:
            intensities = [1, 0.5]
        ...
        # Team leader posts.
        tp = np.array([pos[0, :2], pos[self.ts, :2]]) # Leader xy.
        hl = self.env_size * self.height_lim
        pl.add_mesh(
            pv.lines_from_points(np.asarray([[*tp[0], 0], [*tp[0], hl]])), 
            color=attitude_colors[0], 
            line_width=2
        )
        if not ONE_SWARM:
            pl.add_mesh(
                pv.lines_from_points(np.asarray([[*tp[1], 0], [*tp[1], hl]])), 
                color=attitude_colors[self.ts], 
                line_width=2
            )
        # Labels.
    ...
    def render_vtk(
        ...
        # If lite rendering, return a basic perspective view only.
        if RNDR_LITE:

...
class RecurrentActor(nn.Module):
    """Recurrent Actor Network with a low-bandwidth memory bottleneck."""
    ...
    def __call__(
        ...
        if is_graph_observation(observation):
            ...
            pre_embedding = self.pre_torso(observation)
            action_mask = observation.observation.action_mask
        else:
            pre_embedding = self.pre_torso(observation.agents_view)
            action_mask = observation.action_mask

        mem_in = nn.LayerNorm()(pre_embedding)
        mem_in = nn.Dense(
            self.hidden_state_dim,
            kernel_init=orthogonal(jnp.sqrt(2.0)),
        )(mem_in)

        policy_hidden_state, mem_out = ScannedRNN(self.hidden_state_dim)(
            policy_hidden_state, (mem_in, done)
        )
        policy_embedding = self.post_torso(jnp.concatenate([pre_embedding, mem_out], axis=-1))
...
class RecurrentValueNet(nn.Module):
    """Recurrent Critic Network with a low-bandwidth memory bottleneck."""
    ...
    def __call__(
        ...
        observation_done: Union[RNNObservation, RNNGlobalObservation],RecurrentValueNet
        ...
        value_net_hidden_state, mem_out = ScannedRNN(self.hidden_state_dim)(
            value_net_hidden_state, (mem_in, done)
        )
        value_embedding = self.post_torso(jnp.concatenate([pre_embedding, mem_out], axis=-1))
        value = nn.Dense(1, kernel_init=orthogonal(1.0))(value_embedding)

        return value_net_hidden_state, jnp.squeeze(value, axis=-1)


class ArrayRecurrentValueNet(nn.Module):
    """Recurrent Value Network for array-native critic observations."""
    ...
    def __call__(
        self,
        value_net_hidden_state: chex.Array,
        observation_done: Tuple[chex.Array, chex.Array],
    ) -> Tuple[chex.Array, chex.Array]:
        """Forward pass for critic observations already formatted as arrays."""
        observation, done = observation_done

...
def save_render(filename, frames, frate, verbose: bool = False):
    if verbose:
        print(f"[render-debug] save_render start filename={filename} num_frames={len(frames)}", flush=True)
    ...
    out.release()
    if verbose:
        print("[render-debug] mp4 writer released; starting ffmpeg compression", flush=True)
    ...
    if verbose:
        print("[render-debug] ffmpeg compression finished", flush=True)
...
def final_episodes(config: DictConfig,
    ...
    vprint(f"render_config max_steps={render_config.env.scenario.task_config.max_steps} num_final_episodes={render_config.env.scenario.task_config.num_final_episodes}")

    env, _ = environments.make(config=render_config, add_global_state=False)
    setattr(env, 'render_debug', True)
    ...
    pbar = tqdm(total=num_episodes, desc=f'Generating states for {num_episodes} final episodes')
    (key, *reset_keys) = jax.random.split(key, num_episodes + 1)
    for ep in range(num_episodes):
        vprint(f"episode {ep+1}/{num_episodes}: reset start")
        state, timestep = reset_fn(reset_keys[ep], tot_steps=0)
        base_state = unwrap_env_state(state)
        vprint(f"episode {ep+1}: reset done step={int(base_state.step)} state_type={type(base_state).__name__} hmap_shape={tuple(np.asarray(base_state.hmap).shape)}")
        step = 0
        while not timestep.last():
            step += 1
            if step <= 3 or step % 25 == 0:
                vprint(f"episode {ep+1}: rollout step {step} begin")
            ...
            if step <= 3 or step % 25 == 0:
                vprint(f"episode {ep+1}: rollout step {step} stored base_step={int(base_state.step)} done={bool(timestep.last())}")

            if step == 1 and ep > 0:
                vprint(f"episode {ep+1}: creating transition states")
        ...
        if states:
            ...
            steps.pop()
        pbar.update(1)
        vprint(f"episode {ep+1}: rollout finished collected_states={len(states)}")
    pbar.close()

    azim = 0.0
    deg_per_step = 0.5
    frames = []
    vprint(f"starting render loop num_states={len(states)}")
    for frame_idx, (state, (step_info, is_transition)) in enumerate(zip(states, steps), start=1):
        vprint(f"frame {frame_idx}/{len(states)} start azim={azim:.2f} transition={is_transition} step='{step_info}'")
        try:
            frame = np.asarray(env.render_vtk(state, step_info, azim, verbose=True)).astype(np.uint8)
        except Exception as e:
            vprint(f"frame {frame_idx} python exception: {type(e).__name__}: {e}")
            traceback.print_exc()
            raise
        frames.append(frame)
        vprint(f"frame {frame_idx} rendered frame_shape={frame.shape}")
        azim += deg_per_step * (0 if is_transition else 1)

    vprint("all frames rendered; writing video")
    save_render('test.mp4', frames, framerate, verbose=True)
